      ...
      ...contextBlock?.show
        ? [
          new PromptOption(t('common.promptEditor.context.item.title'), {
            icon: <RxFile className='w-4 h-4 text-[#6938EF]' />,
            onSelect: () => {
      ...
      ...queryBlock?.show
        ? [
          new PromptOption(t('common.promptEditor.query.item.title'), {
            icon: <RxPencil2 className='w-4 h-4 text-[#FD853A]' />,
            onSelect: () => {
      ...
      ...historyBlock?.show
        ? [
          new PromptOption(t('common.promptEditor.history.item.title'), {
            icon: <RxCounterClockwiseClock className='w-4 h-4 text-[#DD2590]' />,
            onSelect: () => {
...
export const useVariableOptions = (
  ...
  const options = useMemo(() => {
    const baseOptions = (variableBlock?.variables || []).map((item) => {
      return new VariableOption(item.value, {
        icon: <RxCode className='w-[14px] h-[14px] text-[#2970FF]' />,
        onSelect: () => {
  ...
  const addOption = useMemo(() => {
    return new VariableOption(t('common.promptEditor.variable.modal.add'), {
      icon: <RxCode className='mr-2 w-[14px] h-[14px] text-[#2970FF]' />,
      onSelect: () => {
        editor.update(() => {
...
export const useExternalToolOptions = (
  ...
  const addOption = useMemo(() => {
    return new VariableOption(t('common.promptEditor.variable.modal.addTool'), {
      icon: <RxMix className='mr-2 w-[14px] h-[14px] text-[#444CE7]' />,
      extraElement: <RxArrowTopRight className='w-3 h-3 text-gray-400' />,
      onSelect: () => {
